...
class FunctionNode(Node):
    ...
    def __init__(self,name, params, block, output):
        ...
        self.block = block
        self.output = output
        func_table[self.name.value] = self
...
class FunctionCall(Node):
    ...
    def evaluate(self):
        global symbol_table
        old_dic = symbol_table
        global func_table
        func_node = func_table[self.name.value]
        new_dic = {}
        for i in range(len(func_node.params)):
            new_dic[func_node.params[i].value] = self.args[i].evaluate()
        old_dic = symbol_table
        symbol_table = new_dic
        func_node.evaluate()
        result = func_node.output.evaluate()
        symbol_table = old_dic
        return result
...
def p_functionlist_function(t):
    '''functions : function'''
    t[0] = [t[1]]

def p_function(t):
    '''function : FUN SYMBOL LPAREN params RPAREN ASSIGN block expression SEMICOLON'''
    t[0] = FunctionNode(t[2], t[4], t[7], t[8])

def p_params(t):
    '''params : params COMMA SYMBOL'''
    t[0] = t[1] + [t[3]]
...
def p_function_call(t):
    '''function_call : SYMBOL LPAREN args RPAREN'''
